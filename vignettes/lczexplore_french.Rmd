...
On passera le vecteur contenant les valeurs possibles à l'argument `LCZlevels` et les couleurs leur correspondant à l'argument `colors`.

Si `repr=alter` et que `LCZlvels` et `colors` ne sont pas renseignés les couleurs sont choisies au sein de la palette "Polychrome 36" du package grDevice, et les niveaux déduits des valeurs présentes.
...
On peut également renseigner le vecteur des couleurs qu'on souhaite utiliser en renseignant l'argument`colors`.On peut renseigner les noms de couleurs connues de R ou un code RGB.  
