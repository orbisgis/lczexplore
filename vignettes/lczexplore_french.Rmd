...
Il est aussi possible de l'installer depuis le dépôt github. Ce dépôt sera modifié lorsque la première version release sera jointe au dépôt officiel de l'équipe Orbisgis, cette version temporaire peut être téléchargée depuis le github du créateur du paquet (`devtools::install_github("orbisgis/lczexplore", build_vignettes = TRUE)`).
...
L'argument `repr=standard` permet de gérer les niveaux de LCZ les plus usuels pour des cartes mises en forme avec des couleurs standardisées.
...
Dans le cas où on a procédé à un regroupage de modalités, si les LCZ ont un encodage particulier, ou si on veut utiliser la fonction pour représenter d'autres variables, on précisera `repr="alter` et on passera le vecteur contenant les valeurs possibles à l'argument `LCZlevels` et les couleurs leur correspondant à l'argument `cols`.
...
#showLCZ(sf=redonOSM,wf="OSM",column="LCZ_PRIMARY",repr="alter")
showLCZ(sf=redonOSM,wf="OSM",column="LCZ_PRIMARY",repr="alter")
