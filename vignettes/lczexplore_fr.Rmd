...
L'import des fichiers .fgb (format recommandé), .geojson et .shp a été testé, mais tous les formats acceptés par la fonction
...
```{r, import-des-fichiers, eval = TRUE, echo = TRUE}
dirPath <- system.file("extdata/multipleWfs/Redon", package = "lczexplore")
redonOSM <- importLCZvect(dirPath = dirPath, file = "osm_lcz.fgb", column = "LCZ_PRIMARY", verbose = FALSE)
redonBDT <- importLCZvect(dirPath = dirPath, file = "bdt_lcz.fgb", column = "LCZ_PRIMARY", verbose = FALSE)
```
...
```{r, show, echo=FALSE, fig.dim = c(8, 6)}
...
```{r, show-drop, fig.dim = c(8, 6), echo = FALSE}
...
```{r, show-alter, fig.dim = c(8, 6), eval = TRUE, echo = TRUE}
...
```{r, niveau-vide, fig.dim = c(8, 6), eval = TRUE, echo = TRUE}
...
```{r, couleurs-nommees, fig.dim = c(8, 6), eval = TRUE, echo = TRUE}
...
```{r, groupage, fig.dim = c(8, 6), eval = TRUE, echo = TRUE}
...
```{r, comparaison, fig.dim = c(16,12 ), eval = TRUE, echo = FALSE}
path <- paste0(system.file("extdata/", package = "lczexplore"), "/redonCompare")
...
```{r, comparaison-image-jolie, out.width = "680px"}
...
```{r, valeurs-numeriques, eval = TRUE, echo = TRUE}
comparaison$percAgg
comparaison$pseudoK
```
...
```{r, analyse-sensibilite, eval = TRUE, echo = TRUE }
dirPath <- paste0(system.file("extdata", package = "lczexplore"), "/multipleWfs/Redon")

redonOSM <- importLCZvect(dirPath = dirPath, file = "osm_lcz.fgb", column = "LCZ_PRIMARY", geomID = "ID_RSU", confid = "LCZ_UNIQUENESS_VALUE")

redonBDT <- importLCZvect(dirPath = dirPath, file = "bdt_lcz.fgb", column = "LCZ_PRIMARY", geomID = "ID_RSU", confid = "LCZ_UNIQUENESS_VALUE")
...
```{r, comparaison-avec-confiance, echo = TRUE, fig.show="hide"}
path <- system.file('extdata/redonCompare',package="lczexplore")
redonCompare <- compareLCZ(sf1 = redonBDT, wf1 = "bdt", geomID1 = "ID_RSU", column1 = "LCZ_PRIMARY",
                           confid1 = "LCZ_UNIQUENESS_VALUE",
                           sf2 = redonOSM, wf2 = "osm", geomID2 = "ID_RSU", column2 = "LCZ_PRIMARY", confid2 = "LCZ_UNIQUENESS_VALUE", exwrite = FALSE, saveG = path)
# redonInputSensib<-read.csv()
```
```{r, sortie-jolie, out.width = "680px"}
path <- system.file('extdata/GeneralUniquenessSensib.png',package="lczexplore")
knitr::include_graphics(path, error=FALSE)

```
Il est important de noter que les colonnes contenant les LCZ en entrée sont renommées à partir des
noms de workflow en sortie, pour éviter des renommages du type lcz_primary et lcz_primary.1.
...
```{r, analyse-sensibilite-2, fig.dim = c(12,9), fig.show="hide"}
...
confidRedon <- confidSensib(inputDf = redonCompare$data, filePath = "", nPoints = 10,
                            ...
                            sep = ";", repr = "standard",
                            plot = TRUE, saveG = system.file("extdata/", package = "lczexplore"))

```
```{r, sortie-sensibilite-jolie, out.width = "680px"}
path <- paste0(system.file("extdata/GeneralUniquenessSensib",package="lczexplore"), ".png")
knitr::include_graphics(path, error=FALSE)

```
La fonction `confidSensib` propose également cette analyse par type de LCZ. 

```{r, sensibilite-par-type-de LCZ, out.width = "680px"}
path <- paste0(system.file("extdata/byLCZUniquenessSensib", package = "lczexplore"), ".png")
knitr::include_graphics(path, error = FALSE)
...
```{R cleanup}
path <- system.file("extdata/byLCZUniquenessSensib.png", package = "lczexplore")
if (file.exists(path)){file.remove(path)}

path <- system.file("extdata/GeneralUniquenessSensib.png", package = "lczexplore")
if (file.exists(path)){file.remove(path)}
path <- system.file("extdata/comparison.png", package = "lczexplore")
if (file.exists(path)){file.remove(path)}
```