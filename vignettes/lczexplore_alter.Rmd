...
The `groupLCZ` function allows this grouping, and the main functions of the package allow to specify the expected levels and colors to use.  
...
The package allows quite a flexibility to the user, thanks to the `levCol` function. 
This function is designed to be called by `showLCZ` and `compareLCZ` functions, not by the user, 
but one can use it to see how the levels and colors are dealt with. It outputs a vector of levels and associated colors, 
as well as a message, explaingin the case it dealt with. 
...
If one know the number of levels of a qualitative variable but not their names,
in an exploratory approach, one can simply specify a vector of prefered colors. 
...
The `levCol` funbction will then replace it with a color chosen from a standard palette. 
...
```
All the cases are not listed in this vignette as the message are hopefully explicit enough. 

# A trick to group and compare all at once

Sometimes, one wants to compare different grouping of levels from the same original pair of datasets. 
In this case, one can do the grouping and the comparing in one step. One needs to pas sthe original column names, 
...
```
This is meant as an exploratopry tool, as the resulting grouped columns are not actually kept after the analysis. 
