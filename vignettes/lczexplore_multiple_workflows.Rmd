...
In case of several workflows and several locations, the function `importMultipleLocsLCZvect` allow to load all the
data in a list.
...
The function `importMultipleLocsLCZvect` loads all the input .fgb files in a list.

When comparing maps for only one location use the similar function `importMultipleLCZvect`.
...
It is also directly given as an output of compareMultipleLCZ()

```{r, multiple-pairwise-agreement}
twoLocsCompared$agreements
```
...
```{r, chord-diagram, fig.width=16, fig.height=10}
drawChordDiagram(weightedFluxIn = twoLocsFlow)
```
This treatment is also directly done in compareMultipleLCZ, but as circlize doesn't create an object (like ggplot2 does) it can not be exported as an object, so you have to recall `drawChordDiagram`.

### A very specific case of missing geometries
When, for a location, some workflows have missing geometries, i.e. don't map some area and leave them as holes,
one can put a zone.fgb file in the directory, and the function `importConcatMultipleLocsLCZvect` will allow to use it
...
# allWfsAllLocs<-importConcatMultipleLocsLCZvect(twoLocsDir, locations = NA, workflowNames = c("osm", "bdt", "wudapt"),
