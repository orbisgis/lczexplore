...
matConfLCZGlob <- function(filePath = "", inputDf, wf1, wf2, geomID1 = "", column1, confid1 = "",
  ...
  areaLCZ1 <- intersec_sf %>%
    dplyr::group_by_at(.vars = column1) %>%
  ...
  areaLCZ2 <- intersec_sf %>%
    dplyr::group_by_at(.vars = column2) %>%
    dplyr::summarize(area = sum(area, na.rm = F)) %>%
    drop_units %>%
    ungroup()
  ...
  matConf <- intersec_sf %>%
    dplyr::group_by_at(.vars = c(column1, column2)) %>%
