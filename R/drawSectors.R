...
#'
#' @param sectorID ar the identifiers of sectors as returned by the makeSectorsAndGroups function
...
#'
#' @export
drawSectors <- function(sectorID, sectorsIn = sectors,
                        colorMapIn = colorMap, textMatch){

  sectorsToHighlight <- grep(x = sectorsIn, pattern = sectorID, value = T) %>% unique
  textOut <- textMatch[sectorID]
  ...
  highlight.sector(sectorsToHighlight,
                   track.index = 1, col = colorMapIn[sectorID],
                   text = textOut, text.col = shades::complement(colorMapIn[sectorID]),
