...
- the raster stream processes remotely sensed information, and applies machine learning
  ...
  but also large-scale maps for Europe, the continental United States and the whole world
  [@demuzere2019mapping; @demuzere2020combining; @demuzere2022global].
...
- only raster maps where pixels match perfectly (same size, not translated) can be treated, or
  some pre-treatment are needed (like nearest neighbours interpolation for instance).
...
for LCZs, or sensitivity analysis according to confidence) justified the development of `lczexplore`.
...
all the urban LCZ types and another for landscape LCZ types,
...
Some algorithms add a uniqueness value to the LCZ type it assigns to a spatial unit.
It can measure if another LCZ level could have been assigned to this unit. Thus, it can be seen
...
- `$data`, intersected geometries, their identifiers and associated confidence value
  (if fed to the function), their LCZ type and their area,
...
Urban Typology by Random Forest [@bocherGeoprocessingFrameworkCompute2018] are available as examples in this package.
