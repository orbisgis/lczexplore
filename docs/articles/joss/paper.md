...
The four main reasons to compare categorical variable son geographical units are :     
...
The terms rural and urban lack of a clear definition and several different classifications 
...
Several papers describe methods to produce LCZ classifications, but there are fewer workflows that automatically 
produce them from available data.
...
several LCZ types could be assign to the corresponding geometries. Therefore, some algorithms associate a confidence 
...
3.  The two LCZ classifications are then compared :
    ...
    2.  The general agreement and a pseudo-kappa indicator of agreement are computed. 
    3.  The summed surface of each LCZ type is computed for each classification
    4.  A confusion matrix shows how the levels of one LCZ classification break up into the levels of the other.
...
either totally agree either totally disagree, and this prevents artificial rounding effects.
...
- a pseudo kappa statistics of this agreement, that may better take into account random agreement   
...
The workflow of comparison of LCZ maps can be used for any pair of maps of categorical variables, under certain limitations : 
...
For each LCZ type, the x-axis shows the minimum confidence threshold.  
...
This paradoxical result shows the limits of the algorithm used to predict building heights in OSM, 
and illustrates the interest of these types of sensitivity analysis.
...
The function `importQualVar` allows you to import maps of othe qualitative variable on (multi)polygons. 
